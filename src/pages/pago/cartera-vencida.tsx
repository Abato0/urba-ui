...
export const ListadoCarteraVencida = () => {
  ...
  const ExportExcel = () => {
    if (isNotNilOrEmpty(dataColumnsV2) && isNotNilOrEmpty(keysColumns)) {
      ...
      XLSX.utils.book_append_sheet(workBook, workSheet, "Carteria Vencida");
...
ListadoCarteraVencida.getLayout = function getLayout(page: React.ReactElement) {
  return (
    <div>
      <NavBar />
      <AppLayout titulo="Pagos - Catera Vencida">{page}</AppLayout>;
    </div>
  );
};
